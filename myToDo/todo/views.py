...
def current_todos(request):
    ...
    print(data)
    return render(request, 'todo/current_todos.html', {'datas': data if data else 'На даннный момент у Вас нет активных задач. Хотите создать?'})


def view_completed_todo(request):
    data = ToDo.objects.filter(user=request.user, deadline__isnull=False).order_by('-deadline')
    return render(request, 'todo/completed_todo.html', {'datas': data})
